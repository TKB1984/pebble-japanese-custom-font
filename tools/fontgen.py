...
class Font:
    def __init__(self, ttf_path, height, max_glyphs, legacy):
        ...
        try:
            self.name = self.face.family_name.decode('utf-8') + "_" + self.face.style_name.decode('utf-8')
    ...
    def set_regex_filter(self, regex_string):
        if regex_string != ".*":
            try:
                self.regex = re.compile(regex_string, re.UNICODE)
    ...
    def is_supported_glyph(self, codepoint):
        return (self.face.get_char_index(codepoint) > 0 or (codepoint == chr(self.wildcard_codepoint)))
